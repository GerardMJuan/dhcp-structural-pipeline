...
Options:
  ...
  -rerunsurf  <number>          Rerun all the pipeline with corrected vtk pial and white values if 1 (default 0).
...
# Print rerunsurf variable to check its value
echo "rerunsurf variable value: $rerunsurf"

# Remove all the files except the ones generated by recon-neonatal-cortex
if [ $rerunsurf -eq 1 ]; then
  find $outwb $outvtk $outtmp -type f ! -name "$subj.*.pial.native.surf.vtk" ! -name "$subj.*.white.native.surf.vtk" -exec rm {} +
fi
...
########### TODO DEBUG BLA BLA BLA
echo "Using $surface_recon_config_tissue"
surface_recon_config=$surface_recon_config_tissue
####### REMOVE REMOVE REMOVE REMOVE

if [ $completed -eq 0 ] && [ ! -f "$outvtk/$subj.*.pial.native.surf.vtk" ] && [ ! -f "$outvtk/$subj.*.white.native.surf.vtk" ]; then
